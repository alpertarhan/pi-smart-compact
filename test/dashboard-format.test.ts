...
describe("dashboard format helpers", () => {
  ...
  it("keeps dashboard highlighting limited to explicit title lines", () => {
    ...
    expect(isDashboardTitleLine("Quality drilldown")).toBe(true);
    expect(isDashboardTitleLine("Provider routes")).toBe(true);
    expect(isDashboardTitleLine("Canary / stable control")).toBe(true);
    expect(isDashboardTitleLine("Overview report")).toBe(false);
  });

  it("shows route and repair provenance in run details", () => {
